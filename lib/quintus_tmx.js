...
Quintus.TMX = function(Q) {
 ...
 Q.loadTMX = function(files,callback,options) {
   ...
   Q.load(files,function() {
     ...
     if(additionalAssets.length > 0) {
        if(options.tmxImagePath){ 
            additionalAssets.forEach(function(asset,i){
                additionalAssets[i] = asset.replace(/^(?:\.\.\/)+/, "").replace("images/","");
                additionalAssets[i] = options.tmxImagePath+additionalAssets[i];
            });
        }
 ...
 Q._tmxLoadTilesets = function(tilesets, tileProperties) {
   ...
   for(var t = 0; t < tilesets.length;t++) {
        var parser = new DOMParser();
        var assetName = "maps/"+Q._tmxExtractAssetName(tilesets[t]);
        var tileset = parser.parseFromString(Q.assets[assetName], "application/xml").childNodes[0];
     ...
     Q.sheet(sheetName, Q._tmxExtractSources(tileset)[0],  tilesetProps);
   }
   return gidMap;
 };
 ...
 Q._tmxProcessTileLayer = function(stage, gidMap, tileProperties, layer) {
    var tiles = layer.childNodes[1].innerHTML.split(","),
   ...
   for(var y=0;y<height;y++) {
     ...
     for(var x=0;x<width;x++) {
       var gid = parseInt(tiles[idx]) - 1;
       if(gid === 0) {
         data[y].push(null);
       } else {
         if(!gidOffset) {
           gidDetails = Q._lookupGid(gid, gidMap);
 ...
 Q._tmxProcessObjectLayer = function(stage,gidMap,tileProperties,layer) {
   ...
   for(var i=0;i < objects.length;i++) {
     ...
     if(!overrideProperties) { throw "Invalid TMX Object: missing properties for GID:" + gid; }
     if(!overrideProperties['Class']) { throw "Invalid TMX Object: missing Class for GID:" + gid; }

     var className = overrideProperties['Class'];
     if(!className) { throw "Invalid TMX Object Class: " + className + " GID:" + gid; }

     var p = Q._extend({ x: x, y: y }, overrideProperties)
